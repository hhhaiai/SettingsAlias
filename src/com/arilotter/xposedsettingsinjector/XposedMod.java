...
public class XposedMod implements IXposedHookLoadPackage, IXposedHookZygoteInit {
    ...
    public void handleLoadPackage(final LoadPackageParam lpparam) throws Throwable {
        if (lpparam.packageName.equals("com.android.settings")) {
            ...
            Class<?> DashboardSummary = findClass("com.android.settings.dashboard.DashboardSummary",
                    lpparam.classLoader);
            ...
            try {
                findAndHookMethod(DashboardSummary, "updateTileView", Context.class, Resources.class, DashboardTile,
            ...
            try {
                if (Build.VERSION.SDK_INT >= 23)
                    findAndHookMethod(SettingsActivity, "loadCategoriesFromResource", int.class, List.class,
                            Context.class, loadCategoriesFromResourceHook);
                else
                    findAndHookMethod(SettingsActivity, "loadCategoriesFromResource", int.class, List.class,
                            loadCategoriesFromResourceHook);
            } catch (Throwable t) {
            ...
            try {
                findAndHookMethod("de.robv.android.xposed.installer.util.ModuleUtil", lpparam.classLoader,
                        "updateModulesList", boolean.class, new XC_MethodHook() {
                            @Override
    ...
    private XC_MethodHook loadCategoriesFromResourceHook = new XC_MethodHook() {
        ...
        protected void afterHookedMethod(MethodHookParam param) throws Throwable {
            ...
            if (installerInfo != null)
                xposedModulesList.add(0, installerInfo); // Always add the
                                                         // Xposed installer as
                                                         // the first item
    ...
    private void setIconAndTint(XC_MethodHook.MethodHookParam param) {
        ...
        if (Build.VERSION.SDK_INT >= 23) {
            ...
            if (bool != null && (boolean) bool) {
                ...
                try {
                    Class<?> clazz = Class.forName("android.graphics.drawable.Drawable");
                    Method method = clazz.getMethod("checkSelfPermission", android.content.res.ColorStateList.class);
                    int rest = (Integer) method.invoke(icon, null);
                    if (rest == PackageManager.PERMISSION_GRANTED) {
                        Log.d("sanbo", "Drawable.setTintList() process success");
                    } else {
                        Log.e("sanbo", "Drawable.setTintList()  process failed");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
